...
export default function Page() {
  ...
  if (isLoading) return <div className='min-h-screen flex items-center justify-center'>กำลังโหลดหน้า Website{dots}</div>;
          ...
          {showAll && (
            <div ref={popupRef} className='absolute bottom-[80px] right-0 w-[320px] sm:w-[380px] bg-red-50 border border-red-300 shadow-lg rounded-lg p-5 backdrop-blur-md'>
              ...
              <div className='flex flex-col gap-2 mb-2 max-h-[200px] overflow-y-auto'>
                {(showFullList ? lowStockIngredients : lowStockIngredients.slice(0, 4)).map((ingredient: DetailIngredient) => (
                  <Badge key={ingredient.ingredient_id} variant='destructive' className='text-sm w-fit'>
                    {ingredient.ingredient_name} ({Number(ingredient.ingredient_total)} / {Number(ingredient.ingredient_total_alert)})
                  </Badge>
                ))}
              </div>
              {lowStockIngredients.length > 4 && (
                <button onClick={() => setShowFullList(!showFullList)} className='text-base font-medium text-red-600 hover:underline'>
          ...
          {menuItems.map((item) => (
            <Card key={item.id} className='group hover:shadow-xl transition-all duration-300'>
              <CardContent className='relative p-0'>
                ...
                <Button variant='ghost' onClick={item.onClick} className='w-full h-24 ml-2 flex items-center justify-start space-x-5 px-7 text-foreground font-semibold hover:bg-transparent'>
                  <div className={`w-14 h-14 ${item.color.bg} ${item.color.hover} rounded-xl flex items-center justify-center transition-all duration-300`}>
