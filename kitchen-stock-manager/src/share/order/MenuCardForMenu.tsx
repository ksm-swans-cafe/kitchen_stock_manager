        ...
        <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
          <div className='bg-white p-6 rounded-lg shadow-lg w-full max-w-md max-h-[70vh] overflow-auto'>
            ...
            {loading ? (
              ...
              <ul className='mb-4 max-h-48 overflow-y-auto'>
                {filteredMenuList.map((menu) => (
                  <li key={menu.menu_id} className='border-b py-3'>
                    <div className='flex items-center justify-between mb-2'>
                      <span className='font-medium'>{menu.menu_name}</span>
                      <div className='flex items-center gap-2'>
                        <button onClick={() => handleDecrease(menu.menu_id)} className='px-2 py-1 rounded bg-gray-300 text-gray-700 font-bold hover:bg-gray-400 transition'>
                        ...
                        <input
                          ...
                          min={0}
                        />
                        <button onClick={() => handleIncrease(menu.menu_id)} className='px-2 py-1 rounded bg-gray-300 text-gray-700 font-bold hover:bg-gray-400 transition'>
            ...
            <div className='flex justify-between mt-6'>
              <button onClick={handleCancel} style={{ backgroundColor: "#dc2626", color: "#ffffff" }} className='px-6 py-2 rounded font-bold hover:bg-red-700 transition'>
                Cancel
              </button>
              <button onClick={handleConfirm} style={{ backgroundColor: "#16a34a", color: "#ffffff" }} className='px-6 py-2 rounded font-bold hover:bg-green-700 transition'>
                Confirm
              </button>
            </div>
          </div>
