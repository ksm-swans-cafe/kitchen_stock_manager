...
const SummaryList: React.FC = () => {
  ...
  const { data: cartsData, error: cartsError, mutate: mutateCarts } = useSWR("/api/cart/lists?page=summarylist", fetcher, { refreshInterval: 30000 });
  ...
  const handleSaveTimes = async (cartId: string) => {
    ...
    try {
      ...
      const response = await api.patch(`/api/cart/time/${cartId}`, payload);
  ...
  const handleToggleIngredientCheck = async (cartId: string, menuName: string, ingredientName: string) => {
    ...
    try {
      const response = await api.patch(`/api/cart/menu/ingredient-status/${cartId}`, {
        menuName,
        ingredientName,
        isChecked: newCheckedStatus,
      });
  ...
  const handleCheck = {
    AllIngredients: async (cartId: string) => {
      ...
      try {
        const response = await api.patch(`/api/cart/menu/all-ingredients-status/${cartId}`, { isChecked: true });
        mutateCarts();
      ...
      try {
        await Promise.all(
          targetCarts.map(async (cart) => {
            const response = await api.patch(`/api/cart/menu/all-ingredients-status/${cart.id}`, { isChecked: true });
          })
        );
                                                    ...
                                                    if (editMenuDialogTimes) {
                                                      ...
                                                      if (timesChanged) {
                                                        try {
                                                          ...
                                                          const response = await api.patch(`/api/cart/time/${editMenuDialog.id}`, payload);
