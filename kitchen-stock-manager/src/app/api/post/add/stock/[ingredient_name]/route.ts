...
// http://localhost:3000/api/post/optional/stock/หัวหอม
import { NextRequest, NextResponse } from "next/server";
import sql from "@app/database/connect";

export async function POST(
  req: NextRequest,
  { params }: { params: { ingredient_name: string } }
) {
  ...
  try {
    ...
    if (!username || !add_decrease || !total_price) {
      return NextResponse.json(
        { error: "Username, add/decrease, and total price are required." },
        { status: 400 }
      );
    }
    ...
    if (!quantity || !unit) {
      return NextResponse.json(
        { error: "Quantity and unit are required." },
        { status: 400 }
      );
    }
