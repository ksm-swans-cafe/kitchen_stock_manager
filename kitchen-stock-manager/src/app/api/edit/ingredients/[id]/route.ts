...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }): Promise<NextResponse> {
  ...
  try {
    ...
    if (ingredient_total !== null && (!Number.isFinite(Number(ingredient_total)) || Number(ingredient_total) <= 0)) {
      return NextResponse.json({ error: "Ingredient total must be a positive number" }, { status: 400 });
    }

    if (ingredient_unit && !ingredient_unit.trim()) {
      return NextResponse.json({ error: "Ingredient unit cannot be empty" }, { status: 400 });
    }

    if (ingredient_total_alert !== null && (!Number.isFinite(Number(ingredient_total_alert)) || Number(ingredient_total_alert) <= 0)) {
      return NextResponse.json({ error: "Ingredient total alert must be a positive number" }, { status: 400 });
    }
