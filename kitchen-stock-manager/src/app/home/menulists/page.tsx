...
import {
  ...
  IngredientUnit,
  MenuLunchbox,
} from "@/models/menu_list/MenuList";
...
const useMenuLists = create<MenuLists>((set) => ({
  ...
  setMenuLunchbox: (lunchbox: MenuLunchbox[]) =>
    set({ menuLunchbox: lunchbox }),
...
export default function Page() {
  ...
  const { data: ingredientOptions } = useSWR<IngredientOption[]>(
    "/api/get/ingredients",
    fetcher,
    {
  ...
  const { data: lunchboxOptions } = useSWR<LunchboxOption[]>(
    "/api/get/lunchbox",
    fetcher,
    {
      revalidateOnFocus: false,
  ...
  const { data: ingredientUnitsData } = useSWR(
              ...
              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-green-100 text-green-800">
                {lb.lunchbox_cost && (
                  <span className="ml-1 text-green-600">
                    ({lb.lunchbox_cost} บาท)
                  </span>
                )}
              </span>
              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-purple-100 text-purple-800">
                {lb.lunchbox_menu_category && (
                  <span className="ml-1 text-purple-600">
                    ({lb.lunchbox_menu_category})
                  </span>
                )}
  ...
  const getUniqueLunchboxNames = () => {
    const uniqueNames = [
      ...new Set((lunchboxOptions || []).map((option) => option.lunchbox_name)),
    ];
    return uniqueNames;
  };
  ...
  const getLunchboxSetOptions = (lunchboxName: string) => {
    ...
    return (lunchboxOptions || []).filter(
      (option) => option.lunchbox_name === lunchboxName,
    );
  };
            ...
            <div className="overflow-x-auto lg:overflow-x-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th
                      className="px-6 py-4 text-left text-base font-black text-gray-900 uppercase tracking-wider"
                      style={{ color: "#000000" }}
                    >
                      ชื่อเมนู
                    </th>
                  ...
                  {menuItems.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-6 py-12 text-center">
                        <div className="flex flex-col items-center">
                          ...
                          <p className="mt-1 text-sm text-gray-500">
                            เริ่มต้นด้วยการเพิ่มเมนูอาหารใหม่
                          </p>
                        </div>
                      </td>
                    </tr>
                    ...
                    menuItems.map((item) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="max-w-xs">
                            {formatLunchbox(item.menu_lunchbox)}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900">
                            ฿{item.menu_cost?.toLocaleString() || 0}
                          </div>
                        </td>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={menuCost}
                      onChange={(e) => setMenuCost(Number(e.target.value))}
                      placeholder="ราคา"
                      className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                      required
                    />
                        ...
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3 text-black-800">
                          ...
                          <div>
                            ...
                            >
                              ...
                              {getLunchboxSetOptions(lb.lunchbox_name).map(
                                (option, optionIdx) => (
                                  <option
                                    key={optionIdx}
                                    value={option.lunchbox_set_name}
                                  >
